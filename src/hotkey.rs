...
pub enum ManagerMessage<K> {
    StartDetect,
    StopDetect,
    Stop,
    Loopback(ThreadMessage),
    Register(K, Vec<Key>, Box<dyn Fn() + Send>),
    Unregister(K),
}
...
pub struct HotkeyManager<K> {
    /// For sending from the manager to the thread
    manager_sender: Sender<ManagerMessage<K>>,

    /// For sending from the thread to the manager
    pub manager_receiver: Receiver<ThreadMessage>,
}

impl<K: Hash + Eq + Send + 'static> HotkeyManager<K> {
    pub fn new(hotkeys: HashMap<K, (Vec<Key>, Box<dyn Fn() + Send>)>) -> Self {
        ...
        std::thread::spawn(move || {
            ...
            let mut hotkeys: HashMap<K, (Vec<Key>, Box<dyn Fn() + Send>)> = hotkeys;
                            ...
                            EventType::KeyPress(k) => {
                                if let Some(v) = &mut detected {
                                    ...
                                    if !v.contains(&k) {
                                        v.push(k);
                                        thread_sender
                                            .send(ThreadMessage::Detected(v.clone()))
                                ...
                                for (_, (key, f)) in hotkeys.iter() {
                                    if !key.is_empty() && key.iter().fold(true, |v, x| v && pressed_keys.contains(x)) {
                ...
                if let Ok(message) = thread_receiver.try_recv() {
                    match message {
                        ...
                        ManagerMessage::StopDetect => {
                            ...
                            //warn!("KEYS: {:?}", v);
                            thread_sender
                                .send(ThreadMessage::DetectedStopped(v))
                                .expect("Error sending detected keys");
                            detected = None;
                        }
                        ManagerMessage::Stop => {
                            break;
                        }
                        ManagerMessage::Loopback(m) => {
                            thread_sender.send(m).expect("Error looping back message");
                        }
    ...
    pub fn start_detecting(&self) {
        self.manager_sender
            .send(ManagerMessage::StartDetect)
    ...
    pub fn stop_detecting(&self) -> Vec<Key> {
        self.manager_sender
            .send(ManagerMessage::StopDetect)
            .expect("Error sending message");
        loop {
            if let Ok(m) = self.manager_receiver.recv() {
                match m {
                    ThreadMessage::DetectedStopped(v) => {
                        return v;
                    }
    ...
    pub fn stop(&self) {
        self.manager_sender
            .send(ManagerMessage::Stop)
            .expect("Error sending stop signal");
    }

    pub fn register(&self, name: K, keys: Vec<Key>, f: Box<dyn Fn() + Send>) {
        self.manager_sender
            .send(ManagerMessage::Register(name, keys, f))
            .expect("Error sending register request");
    }

    pub fn unregister(&self, name: K) {
        self.manager_sender
            .send(ManagerMessage::Unregister(name))
            .expect("Error sending unregister request");
    }
}
